    ...
    public static Node create(Scenario s, Window owner) {
        ...
        final Label addNodeL = addButtonWithLabel(addNodeForm, "Add node", () -> {
            if (nodeFormV.isValid) {
                ...
                GroceryImplementationApplication.db.persist(() -> s.add(n), s).onFinish(o -> Platform.runLater(() -> nodeList.updateList(s, null, s::getNodes)));
                Platform.runLater(() -> {
        ...
        addButton(pane, "Remove all nodes", () -> {
            GroceryImplementationApplication.db.persist(() -> s.getNodes().clear(), s).onFinish(o -> Platform.runLater(() -> nodeList.updateList(s, null, s::getNodes)));
            ...
            try {
                ...
                importButton.setOnAction(ae -> {
                    try {
                        ...
                        if (validCounter > 0) {
                            GroceryImplementationApplication.db.persist(() -> s.getNodes().addAll(toAdd), s).onFinish(o -> Platform.runLater(() -> {
                                nodeList.updateList(s, null, s::getNodes);
                                MapView.current().updateMarkers(s.getNodes());
                            }));
                            Platform.runLater(() -> {
